...
def throw_rock(m, v0, theta):
    ...
    tf = 2 * v0 * np.sin(theta) / g # in sec
    R = v0**2 * np.sin(2 * theta) / g #in m
    hm = v0**2 * np.sin(theta)**2 / (2 * g) # in m
    vh = v0 * np.cos(theta) # in m/s
    Kh = 0.5 * m * vh**2 # in J
    
    print("For a rock with %5.3f kg mass thrown with %5.3f m at an angle of %6.2f degrees:\n"\
    "Time of the flight is %10.1e s\n The range in x-direction is %10.1e m\n"\
    "Maximum height is %10.1e m\n The speed at maximum height is %10.1e m/s\n"\
    "Kinetic energy at the maximum height is %8.2f j" % (m,v0,theta,tf,R,hm,vh,Kh))
